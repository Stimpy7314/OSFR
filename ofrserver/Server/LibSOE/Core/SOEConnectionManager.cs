...
namespace SOE.Core
{
    ...
    public class SOEConnectionManager
    {
        ...
        public readonly ConcurrentDictionary<int, SOEClient> Clients;
        ...
        public SOEConnectionManager(SOEServer server)
        {
            ...
            Clients = new ConcurrentDictionary<int, SOEClient>();
        ...
        public bool AddNewClient(SOEClient newClient)
        {
            ...
            {
                if (!Clients.TryAdd(newClientId, newClient))
                {
                    Log.FatalFormat("Failed to add client from {0}, (ID: {1})", newClient.GetClientAddress(), newClient.GetClientID());
                    return false;
                }
            }
            else
